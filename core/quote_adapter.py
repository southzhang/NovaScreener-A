...
"""
行情数据源适配器 v3.0
数据源优先级：腾讯API（主）→ QMT SSH → QMT HTTP（降级备用）

v3.0 变更：腾讯API作为主数据源，QMT降级为备用。
原因：腾讯API稳定免费无限制，QMT依赖Windows SSH连接不稳定。
...
def get_quotes(codes, prefer_qmt=False):
    """
    批量获取行情数据
    优先级：腾讯API → QMT SSH → QMT HTTP
    v3.0: 腾讯为主数据源，QMT降级为备用
    ...
    # 腾讯API为主数据源（稳定、免费、无限制、不依赖SSH）
    result = _tencent_get_quotes(codes)
    if result and len(result) > 0:
        # 检查数据时效性：如果超过50%过期，尝试QMT
        stale_count = sum(1 for q in result.values() if _is_stale_quote(q))
        if stale_count < len(result) * 0.5:
            return result

    # QMT降级：HTTP → SSH
    if is_qmt_http_available():
        qmt_result = _qmt_http_get_quotes(codes)
        if qmt_result and len(qmt_result) > 0:
            stale_count = sum(1 for q in qmt_result.values() if _is_stale_quote(q))
            if stale_count < len(qmt_result) * 0.5:
                return qmt_result

    if is_qmt_ssh_available():
        qmt_result = _qmt_ssh_get_quotes(codes)
        if qmt_result and len(qmt_result) > 0:
            return qmt_result
...
def get_kline(code, period="1d", count=250, prefer_qmt=False):
    ...
    if prefer_qmt:
        # QMT降级
        if is_qmt_ssh_available():
            kline = _qmt_ssh_get_kline(code, period, count)
            if kline:
                return kline

        if is_qmt_http_available():
            kline = _qmt_http_get_kline(code, period, count)
            if kline:
                return kline
...
def get_batch_klines(codes, period="1d", count=250, prefer_qmt=False):
    ...
    if prefer_qmt:
        # SSH直连批量K线
        if is_qmt_ssh_available():
            qmt_result = _qmt_ssh_batch_klines(codes, period, count)
            if qmt_result and len(qmt_result) > 0:
                return qmt_result
        ...
        if is_qmt_http_available():
            ...
            for i in range(0, len(qmt_codes), 50):
                batch = qmt_codes[i:i+50]
                try:
                    url = f"{QMT_DATA_URL}/batch_kline?codes={','.join(batch)}&period={period}&count={count}"
                    r = urlopen(url, timeout=30)
                    data = json.loads(r.read().decode())
                    for qmt_code, bars in data.get("data", {}).items():
                        orig_code = code_map.get(qmt_code, qmt_code.split(".")[0])
                        qmt_result[orig_code] = bars
                except:
                    pass
            if qmt_result:
                return qmt_result
...
def get_full_market_quotes(prefer_qmt=False):
    ...
    if prefer_qmt:
        # QMT HTTP直连
        if is_qmt_http_available():
            qmt_result = _qmt_http_full_market()
            if qmt_result and len(qmt_result) > 100:
                return qmt_result

        # QMT SSH直连
        if is_qmt_ssh_available():
            qmt_result = _qmt_ssh_full_market()
            if qmt_result and len(qmt_result) > 100:
                return qmt_result
