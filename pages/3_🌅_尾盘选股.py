...
# 尾盘专属评分（仅在尾盘交易时段14:20后有实际意义）
tail_results = get_tail_recommendations()
tail_stocks = tail_results.get("stocks", [])
tail_stats = tail_results.get("stats", {})
tail_error = tail_results.get("error", "")
...
# ===== 尾盘专属评分（与V10信号独立）=====
if tail_stocks:
    st.html('<h2>🎯 尾盘专属评分</h2>')
    st.caption("基于V10信号的尾盘二次确认 — 独立于选股扫描的尾盘策略")
    
    _tb = sum(1 for t in tail_stocks if t.tail_action == "可进场")
    _tw = sum(1 for t in tail_stocks if t.tail_action == "观察")
    
    tc = st.columns(4)
    _total_stats = tail_stats.get("total", 0)
    _v10_stats = tail_stats.get("v10_candidates", 0)
    _card_data = [
        (tc[0], f"候选 {_total_stats}只", "var(--text-primary)"),
        (tc[1], f"可进场 {_tb}只", "var(--down-color)"),
        (tc[2], f"观察 {_tw}只", "var(--warning-color)"),
        (tc[3], f"V10来源 {_v10_stats}只", "var(--accent)"),
    ]
    for col_item in _card_data:
        with col_item[0]:
            st.html('<div class="dash-card" style="text-align:center;"><div class="dash-card-header">' + col_item[1] + '</div><div class="dash-card-value" style="color:' + col_item[2] + ';">' + col_item[1].split()[-1] + '</div></div>')

    # 进场
    _buy = [t for t in tail_stocks if t.tail_action == "可进场"]
    if _buy:
        st.html(f"<h3>✅ 尾盘可进场 · {len(_buy)}只</h3>")
        for t in _buy:
            _chg_c = "var(--up-color)" if t.pct_change > 0 else "var(--down-color)"
            _g1 = "\U0001f7e2" if t.tail_volume_score >= 15 else "\U0001f7e1"
            _g2 = "\U0001f7e2" if t.tail_momentum_score >= 15 else "\U0001f7e1"
            _g3 = "\U0001f7e2" if t.tail_trend_score >= 15 else "\U0001f7e1"
            _g4 = "\U0001f7e2安全" if t.tail_distribution >= 15 else "\U0001f7e1关注"
            _scores = f"量能{t.tail_volume_score:.0f}({_g1}) · 动量{t.tail_momentum_score:.0f}({_g2}) · 趋势{t.tail_trend_score:.0f}({_g3}) · 出货{_g4}"
            st.html(f"""
<div style="background:var(--bg-card); border:1px solid var(--border-color); border-left:4px solid var(--down-color); border-radius:10px; padding:14px 18px; margin-bottom:10px;">
    <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:8px;">
        <div>
            <span style="font-weight:700; color:var(--text-primary); font-size:1.05em;">{t.name}</span>
            <span style="color:var(--text-secondary); margin-left:6px;">{t.code}</span>
            <span style="color:{_chg_c}; margin-left:10px; font-weight:600;">¥{t.price:.2f} {t.pct_change:+.2f}%</span>
            <span style="color:var(--accent); margin-left:10px; font-weight:700;">{t.total_tail_score:.0f}分</span>
            <span class="tag tag-accent" style="margin-left:8px;">{t.signal_type}</span>
        </div>
        <span style="background:var(--down-color); color:#fff; padding:2px 10px; border-radius:4px; font-size:0.82em; font-weight:700;">✅ {t.tail_action}</span>
    </div>
    <div style="font-size:0.85em; color:var(--text-secondary); margin-top:6px;">{_scores}</div>
    <div style="font-size:0.82em; color:var(--accent); margin-top:4px;">{t.tail_reason}</div>
</div>
""")

    # 观察
    _watch = [t for t in tail_stocks if t.tail_action == "观察"]
    if _watch:
        with st.expander(f"👁️ 尾盘观察 · {len(_watch)}只"):
            for t in _watch:
                st.html(f'<div style="padding:4px 0; font-size:0.88em;"><b>{t.name}</b>({t.code}) ¥{t.price:.2f} {t.pct_change:+.2f}% <span style="color:var(--warning-color);">{t.total_tail_score:.0f}分</span> — {t.tail_reason}</div>')

if tail_error:
    st.warning(f"💡 {tail_error}")
    
    _quick_scan = st.button("🚀 快速扫描 · 获取今日实时信号", type="primary", use_container_width=True, key="tail_quick_scan")
    if _quick_scan:
        with st.status("⏳ 运行全市场V10扫描（约1-2分钟）...", expanded=True) as _qs:
            import json, os
            from datetime import datetime
            from core.scanner import scan_market
            from core.data import clear_data_cache
            
            _qs.write("正在扫描全市场...")
            _results = scan_market(["v10_full"], max_workers=25)
            clear_data_cache()
            
            # 构建watchlist JSON格式（与v10_realtime_scan.py输出一致）
            ...
            for r in _results:
                _stocks.append({
                    "code": r["code"],
                    "name": r["name"],
                    "signal": r.get("signal_type", r.get("strategy", "")),
                    "price": r["price"],
                    "score": r.get("score", 0),
                    "change": r.get("change", 0),
                })
            
            # 写入watchlist JSON（供get_tail_recommendations读取）
            _wl_path = os.path.expanduser("~/.hermes/cache/v10_watchlist.json")
            ...
            _qs.update(label=f"✅ 扫描完成！共{len(_results)}个信号，刷新中...", state="complete")
            st.rerun()
